...
- **型:** `string | string[] | { [entryAlias: string]: string }`

アプリケーションのエントリーポイントで、プロジェクトルートからの相対パスで解決されます。これは、明示的に設定されていない場合の [`build.rolldownOptions.input`](/config/build-options#build-rolldownoptions)、[`build.lib.entry`](/config/build-options#build-lib)、[`build.ssr`](/config/build-options#build-ssr)（`true` の場合）、および [`optimizeDeps.entries`](/config/dep-optimization-options#optimizedeps-entries) のデフォルト値として機能します。

アプリケーションが `index.html` エントリーを使用しない場合に便利で、上記の各オプションで繰り返し指定する代わりに、一度だけエントリーを宣言すればよくなります。
...
:::warning SSR + ESM
SSR ビルドの場合、`build.rolldownOptions.output` で設定された ESM ビルド出力に対して重複排除が機能しません。これを回避するには、ESM のモジュール読み込みのためのプラグインのサポートが改善されるまで、CJS ビルド出力を使用する必要があります。
:::
...
`paths` は、`tsconfig.json` の `files` または `include` にマッチするファイルにのみ適用されます。`"src"` や `"**/*"` のような `include` の指定は TS/JS 拡張子のみにマッチするため、非 JS 拡張子のファイルは明示的にリストに追加する必要があります。これは TypeScript の動作に合わせたものです。例えば、CSS ファイル内（`@import '@/foo.css'` など）で `paths` エイリアスを使用するには、`files` にファイルを追加するか、`include` に明示的な拡張子を追加してください:
...
::: warning Less はサポートされていません
`resolve.tsconfigPaths` は `.less` ファイル内には適用されません。Less はインポートするファイル自体ではなく、そのディレクトリーのみを Vite に渡すため、Vite はそれにマッチする `tsconfig.json` を見つけられません。Less の `@import` には相対パスか [`resolve.alias`](#resolve-alias) を使用してください。
:::
...
- **型:** `Record<string, HtmlAssetSource>`
...
アセットソースとして扱う追加の HTML 要素と属性を定義します。これは `<img src>`、`<video src>`、`<link href>` などの標準要素を含む組み込みリストを拡張します。

アセットを参照するカスタム Web コンポーネントや非標準属性（`data-*` など）を使用する場合に便利です。
