...
このページはランタイムプロバイダー向けです。JavaScript ランタイムを Vite と統合する作者を対象としています。ここでのランタイムとは、変換されたコードが実行される JavaScript エンジンのことであり、Node.js、ブラウザー、Cloudflare の workerd、ワーカースレッドなどが該当します。ランタイムプロバイダーはこれらのランタイムのいずれかに対する統合をパッケージ化し、フレームワーク作者やエンドユーザー（アプリを構築している開発者）が自分で設定する必要がないようにします。

## 環境ファクトリー {#environment-factories}

環境ファクトリーは、エンドユーザーではなく、ランタイムプロバイダーによって実装されることを目的としています。環境ファクトリーは、開発環境とビルド環境の両方でターゲットランタイムを使用する最も一般的なケースで、`EnvironmentOptions` を返します。デフォルトの環境オプションも設定できるため、ユーザーが設定する必要はありません。
...
Vite サーバーと同じ Node.js プロセスで実行されている SSR 環境では、Vite はすぐに使える `HotChannel` として `createServerHotChannel` をエクスポートします:
