...
## 環境ごとのフックとグローバルフック {#per-environment-hooks-and-global-hooks}

プラグインは共有パイプライン上で動作しますが、そのフックはサーバー全体で一度だけ実行されるか、環境ごとに実行されるかによって 2 つのカテゴリーに分かれます。

グローバルフックは、設定されている環境に関係なく、一度だけ呼び出されます。設定の解決や開発・プレビューサーバーのセットアップなど、アプリ全体に関わる処理を担当するため、`this.environment` はこれらには関係ありません。設定解決に関するフックとサーバーに関するフックはグローバルフックです。

環境ごとのフックは各環境に対して一度ずつ呼び出され、コンテキスト内の `this.environment` を通じて現在の環境を公開します。[Rolldown フック](/guide/api-plugin#rolldown-hooks)はすべて環境ごとのフックであり、モジュールを扱うその他の Vite 固有のフックも同様です。ただし、[`perEnvironmentStartEndDuringDev: true` フラグ](#per-environment-state-in-plugins)がない場合、`buildStart` と `buildEnd` はクライアント環境に対してのみ呼び出されることに注意してください。
...
## `configEnvironment` フックを使用した環境の設定 {#configuring-environment-using-the-configenvironment-hook}

- **型:** `(name: string, config: EnvironmentOptions, env: { mode: string, command: 'build' | 'serve', isSsrBuild?: boolean, isPreview?: boolean, isSsrTargetWebworker?: boolean }) => EnvironmentOptions | null | void`
- **種類:** `async`, `sequential`
- **スコープ:** [環境ごと](#per-environment-hooks-and-global-hooks)
...
同じプラグインインスタンスが異なる環境で使用されるため、プラグインの状態は `this.environment` をキーとして管理する必要があります。これは、エコシステムがクライアントモジュールと SSR モジュールの状態が混ざるのを避けるために、`ssr` ブール値をキーとしてモジュールの状態を保持するために既に使用しているパターンと同じです。`Map<Environment, State>` を使用して、各環境の状態を個別に保持できます。後方互換性のため、`buildStart` および `buildEnd` は、`perEnvironmentStartEndDuringDev: true` フラグがない場合、クライアント環境に対してのみ呼び出されることに注意してください。`watchChange` と `perEnvironmentWatchChangeDuringDev: true` フラグについても同様です。
...
## `applyToEnvironment` フックを使用した環境ごとのプラグイン {#per-environment-plugins-using-the-applytoenvironment-hook}

- **型:** `(environment: PartialEnvironment) => boolean | PluginOption | Promise<boolean>`
- **種類:** `async`, `sequential`
- **スコープ:** [環境ごと](#per-environment-hooks-and-global-hooks)
...
- **開発時とビルド時:** プラグインは[環境ごとのフィルタリング](#per-environment-plugins-using-the-applytoenvironment-hook)で共有されます
