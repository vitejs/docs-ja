...
Vite はモジュールパスごとに 1 つの `import.meta.hot.data` オブジェクトを作成します。このオブジェクトは、HMR 中に同じモジュールの連続したインスタンス間で永続化されます。モジュールの実行中に行われた変更、または `hot.dispose` に渡された `data` 引数を通じて行われた変更は、モジュールの次のインスタンスから参照できます。

モジュールが破棄（prune）されると、その `hot.dispose` と `hot.prune` コールバックは現在の data オブジェクトを受け取ります。これらのコールバックが完了すると、Vite はデータをクリアします。そのモジュールが後で再度インポートされた場合、新しい空の data オブジェクトを受け取ります。
